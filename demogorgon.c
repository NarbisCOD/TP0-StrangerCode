...
#define MSJ_MAL_INGRESO "Ingreso no valido, vuelva a intentar\n"
#define MSJ_INGRESO_ACEPTADO "Ingreso aceptado\n"
#define MSJ_RECIBIR_ANIMO "Ingrese el animo de El(Eleven) en un rango entre 1 al 100 inclusive\n"
...
char recibir_confianza(){
	...
	do{
		...
		if((confianza_recibida == CONFIANZA_BAJA) || (confianza_recibida== CONFIANZA_MEDIA) || (confianza_recibida == CONFIANZA_ALTA)){
			printf(MSJ_NIVEL_CONF_ACEPTADO);
		}else{
			printf(MSJ_MAL_INGRESO);
		}
...
float recibir_fuerza_psiquica(int animo_recibido, char confianza_recibida){
	...
	float calcular_fuerza = DEF_FUERZA_PSQUICA;
    ...
    calcular_fuerza = animo_recibido * multiplicador;
